# Preparing the Main Frame

## 1. Preparing the Platform Deck

Place the **Platform Deck** in front of you as shown below.

![Platform Deck.](img/ulchassis01.JPG)

The image below shows the five components we will mount on the **Platform Deck**:

![Locations of parts to be mounted.](img/ulchassis02_NX.JPG)

> ### ***Note that the screw holes have been adjusted in the new design to fit the new powerboard.***

---

## 2. Mounting the VESC

1. **Position the VESC** on the Platform Deck:
   - **Power wires facing AWAY** from you.
   - **Three cables labeled A, B, and C facing TOWARDS** you.

   ![Correct VESC placement.](img/ulchassis03.JPG)

2. **Secure the VESC**:
   - Flip the Platform Deck and VESC over.
   - Use **two diagonal M5 screws** to attach the VESC.

   ![Mounting holes for Powerboard and VESC (bottom view).](img/ulchassis04.JPG)

---

## 3. Mounting the Antenna

1. **Mount the antenna standoffs and cables** at the rear of the VESC.
   - This allows the antenna cables to run beneath the powerboard and connect directly to the **NVIDIA Jetson NX** before installation.
   - Use an **M3 screw** to secure the antenna standoffs and cable.

   ![Mounted antenna on the platform deck.](img/ulchassisNX_02.JPG)

2. **Attach the black antennas**:
   - Rotate them back onto the antenna mount, the same way they were removed.

   ![Attach the antenna mount.](img/ulchassisNX_03.JPG)

---

## 4. Mounting the NVIDIA Jetson NX

1. **Connect the antenna cables first** before mounting the Jetson NX:
   - Flip the Jetson around.
   - **Make sure that the WiFi card and Storage device is intalled on the Jetson before mounting it**
   - Clip **both antenna cables** to the connectors on the Jetson.

   > ***Note that it might be a little hard to clip the connectors. Ensure that the connectors are compatible. (buying the WiFi card and its Antenna as a bundle is a way to ensure that they are compatible)***

   ![Connect antenna cables to Jetson NX](img/ulchassisNX_04.JPG)

2. **Install M3 standoffs for the Jetson**:
   - Screw the **standoffs** into the Platform Deck at the designated Jetson NX mounting holes.
   - Thread the **M3 screw from underneath** and secure it with a **25mm standoff**.

   ![Install Jetson standoffs.](img/ulchassisNX_05.JPG)

3. **Mount the Jetson NX**:
   - Place the Jetson NX onto the **mounted standoffs**.
   - Secure it using **M3 screws** through the four main holes on top of the Jetson.
   - Secure the **Powerboard** to the **25mm standoff** with an **M3 x 8mm screw**.

   ![Mount Jetson NX to the Platform Deck.](img/ulchassisNX_06.JPG)

---

## 5. Mounting the Powerboard

1. Flip the **Platform Deck (with VESC attached) back over**.
2. Use **M3 screws** and thread them from underneath and secure with a **25 or 30mm standoff**.
   - The image below shows the **four mounting Standoffs** for the Powerboard.

   ![View of Powerboard Standoffs.](img/StandoffsForPowerBoard.jpg)

3. Secure the **Powerboard** to the **standoff** with **M3 screws**.

   ![Top view of Powerboard mounted.](img/MountedPowerboard.jpg)

4. **Ensure proper spacing**:
   - There should be a **gap between the Powerboard and VESC**.

---

## 6. Mounting the Lidar

1. The last component to mount is the **Lidar**.
   - This guide uses the **Hokuyo UTM-30LX**.
   - The mounting holes for the **Hokuyo UST-10LX** are slightly different.

   ![Lidar mounting holes.](img/ulchassis20.JPG)

2. **Use four M3 screws** to secure the Lidar **from underneath**.

   ![Lidar mounted on Platform Deck.](img/ulchassis10.JPG)

---
